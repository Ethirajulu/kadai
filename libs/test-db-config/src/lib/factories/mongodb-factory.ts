...
export class MongoDBConnectionFactory {
  async createConnection(config: MongoDBTestConfig): Promise<MongoDBConnection> {
  ...
  private buildConnectionUri(config: MongoDBTestConfig): string {
    ...
    if (config.host && config.host.startsWith('mongodb://') || config.host && config.host.startsWith('mongodb+srv://')) {
      return config.host;
    }
  ...
  async createIndexes(connection: MongoDBConnection, collectionName: string): Promise<void> {
