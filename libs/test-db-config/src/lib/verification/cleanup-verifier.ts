...
export class CleanupVerifier {
  ...
  async verifyCleanup(
    ...
    const targetDatabases =
      databases || Array.from(this.cleanupStrategies.keys());
    const verificationOptions = { ...options };

    try {
      const results = await this.performVerification(
        targetDatabases,
        verificationOptions
      );
  ...
  private async verifyDatabase(
    ...
    try {
      ...
      const enhancedIssues = await this.performEnhancedVerification(
        database,
        baseResult
      );
  ...
  private async verifyMongoDBEnhanced(): Promise<CleanupIssue[]> {
    ...
    try {
      ...
      const currentOps = await connection.database
        .admin()
      ...
      if (this.config.validateIndexes) {
        const collections = await connection.database
          .listCollections()
          .toArray();

        for (const collection of collections) {
          const indexes = await connection.database
    ...
    try {
      ...
      if (maxMemory > 0 && usedMemory > maxMemory * 0.8) {
        issues.push({
          ...
          description: `Memory usage is ${Math.round(
  ...
  private async verifyQdrantEnhanced(): Promise<CleanupIssue[]> {
    ...
    try {
      ...
      for (const collection of collections.collections || []) {
        const collectionInfo = await connection.client.getCollection(
          collection.name
        );
        ...
        if (
          ...
          collectionInfo.points_count > 10000
        ) {
          issues.push({
            type: 'performance_degradation',
            severity: 'info',
  ...
  private async runCustomValidators(database: string): Promise<CleanupIssue[]> {
    ...
    const validators = this.config.customValidators.filter(
      (v) => v.database === database
    );
  ...
  private generateRecommendations(
    ...
    if (issueTypes.has('data_remaining')) {
      recommendations.push(
        'Run additional cleanup operations to remove remaining data'
      );
    }

    if (issueTypes.has('performance_degradation')) {
      recommendations.push(
        'Optimize database performance by addressing identified bottlenecks'
      );
    }

    if (issueTypes.has('connection_issue')) {
      recommendations.push(
        'Review database connection configurations and pooling settings'
      );
    }
    ...
    if (recommendations.length === 0) {
      recommendations.push(
        'All databases appear to be clean and properly configured'
      );
    }
  ...
  private updateMetrics(report: VerificationReport): void {
    ...
    this.metrics.averageVerificationTime =
      (this.metrics.averageVerificationTime *
        (this.metrics.verificationCount - 1) +
    ...
    this.metrics.successRate =
      (this.metrics.successRate * (this.metrics.verificationCount - 1) +
        (isSuccess ? 1 : 0)) /
      this.metrics.verificationCount;
    ...
    report.detailedResults.forEach((result) => {
      result.issues.forEach((issue) => {
        this.metrics.commonIssues[issue.type] =
          (this.metrics.commonIssues[issue.type] || 0) + 1;
      });

      // Update database performance
      this.metrics.databasePerformance[result.database] =
        (this.metrics.databasePerformance[result.database] || 0) +
